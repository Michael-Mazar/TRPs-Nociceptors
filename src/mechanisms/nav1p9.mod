...
NEURON {
	SUFFIX nav1p9
	USEION na READ ena WRITE ina : NONSPECIFIC_CURRENT i previously
	RANGE gbar, ActShift, InactShift, i, A_am9, B_am9, C_am9, A_bm9, B_bm9, C_bm9
}
...
PARAMETER {
	...
	ActShift=-10 (mV) : activation shift
	InactShift=-10 (mV) : inactivation shift
}
...
BREAKPOINT {
	...
	g = gbar * m * h
	i = g * (v-ena)
	ina = i : Added to record directly from channel
}
...
FUNCTION alphah(Vm (mV)) (/ms) {
	alphah=A_ah9/(1+exp((Vm+B_ah9+InactShift)/C_ah9))
}
...
FUNCTION betah(Vm (mV)) (/ms) {
	betah=A_bh9/(1+exp((Vm+B_bh9+InactShift)/C_bh9))
}
