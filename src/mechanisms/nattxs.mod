...
NEURON {
	SUFFIX nattxs
	USEION na READ ena WRITE ina :NONSPECIFIC_CURRENT i
	RANGE gbar, i : ,ena
}
...
PARAMETER {
	gbar = 0.001699 : =18e-9/(100e-12*1e8) (S/cm2) : 18(nS)/100(um)^2
	ena=60 (mV)
...
FUNCTION alpham(Vm (mV)) (/ms) {
	alpham=A_am/(1+exp((Vm+B_am+ActShift)/C_am))
}
...
FUNCTION betam(Vm (mV)) (/ms) {
	betam=A_bm/(1+exp((Vm+B_bm+ActShift)/C_bm))
}
